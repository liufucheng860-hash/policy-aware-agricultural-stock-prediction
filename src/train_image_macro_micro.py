...
# =========================================================
# =========================================================
class ImageMacroMicroModel(nn.Module):
    ...
    def forward(self, x_img, x_macro, x_micro):
        ...
        fused = torch.cat([img_feat, macro_feat, micro_feat], dim=1)
...
def main():
    ...
    history_file = OUTPUT_DIR / f"metrics_image_macro_micro_{suffix}_history.csv"
    ...
    pred_file = OUTPUT_DIR / f"predictions_image_macro_micro_{suffix}.csv"
    test_result_df.to_csv(pred_file, index=False, encoding="utf-8-sig")

    # summary
    summary_df = pd.DataFrame([{
        "model": "image_macro_micro_fusion",
        "target": TARGET,
    ...
    summary_file = OUTPUT_DIR / f"metrics_image_macro_micro_{suffix}_summary.csv"
