...
def main():
    ...
    print("[INFO] Dynamic weighted fusion: weights are learned per sample.")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        history.append({
            ...
            "val_w_image_std": val_metrics["w_image_std"],
            "val_w_macro_std": val_metrics["w_macro_std"],
            "val_w_micro_std": val_metrics["w_micro_std"],
        })
    ...
    print(f"W_image_std : {test_metrics['w_image_std']:.4f}")
    print(f"W_macro_std : {test_metrics['w_macro_std']:.4f}")
    print(f"W_micro_std : {test_metrics['w_micro_std']:.4f}")
    ...
    pd.DataFrame([{
        ...
        "w_image_mean": test_metrics["w_image_mean"],
        "w_macro_mean": test_metrics["w_macro_mean"],
        "w_micro_mean": test_metrics["w_micro_mean"],
        "w_image_std": test_metrics["w_image_std"],
        "w_macro_std": test_metrics["w_macro_std"],
        "w_micro_std": test_metrics["w_micro_std"],
    }]).to_csv(summary_file, index=False, encoding="utf-8-sig")

    # per-sample weights
    weights = test_metrics["weights"]

    weight_df = meta_test.copy()
    weight_df["y_true"] = test_metrics["y_true"]
    weight_df["y_pred"] = test_metrics["y_pred"]
    weight_df["y_prob"] = test_metrics["y_prob"]
    weight_df["w_image"] = weights[:, 0]
    weight_df["w_macro"] = weights[:, 1]
    weight_df["w_micro"] = weights[:, 2]

    weight_file = OUTPUT_DIR / f"dynamic_weights_{suffix}.csv"
    weight_df.to_csv(weight_file, index=False, encoding="utf-8-sig")
